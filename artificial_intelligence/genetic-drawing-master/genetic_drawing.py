'''
La popolazione è fatta di cromosomi (pennellate) che contengono ciascuno: colore, posizione, dimensione, direzione e il tipo di
pennellata (i tipi di pennellata in questo caso sono 4 nella cartella .\brushes\watercolor).
Ogni popolazione costituisce N pennellate (N cromosomi). La popolazione viene fatta evolvere fino a che le pennellate non si
avvicinano il più possibile alla figura da riprodurre (ovviamente c'è un numero massimo di generazioni). 
La popolazione è memorizzata dentro DNASeq (sequenze di DNA, ovvero cromosomi) che è una lista di liste.
In questo codice vengono fatte evolvere più popolazioni utilizzando una dimensione del pennello sempre più piccola. 
Ciò viene effettuato attraverso gli stage: più lo stage è alto e più è piccola la dimensione del pennello.
Viene lasciata dipinta sempre e solo la popolazione migliore dello stage precedente.
La maschera serve a concentrare l'algoritmo genetico su parti dell'immagine che hanno bisogno di più dettagli.
...
class GeneticDrawing:
    ...
    def generate(self, stages=10, generations=100, brushstrokesCount=10, show_progress_imgs=True):
        # Per ogni stage il pennello dei cromosmi si rimpiccolisce
        for s in range(stages):
            ...
            # Istanzia l'oggetto DNA, ovvero l'oggetto che contiene i parametri e la popolazione del GA    
            # La dimensione del pennello di ogni cromosoma dipende da s, ovvero dal numero dello stage
            # Più è alto lo stege e più è piccolo il pennello.
            ...
            # Inizializza randomicamente la popolazione                 
            ...
            # All'immagine precedente viene aggiunta quella corrispondente alla migliore popolazione. 
...
class DNA:
    ...
    # Inizializzazione della popolazione. Il numero dei cromosomi è pari al numero delle pennelate (brushstrokesCount).
    # Ogni cromosoma ha il colore (intensità), la posizione, la dimensione, la direzione e il tipo di pennellata
    # (i tipi di pennellata in questo caso sono 4 nella cartella .\brushes\watercolor). 
    def initRandom(self, target_image, count, seed):
        ...
        for i in range(count):
            ...
            # Numero (id) del pennello (sono 4 tipi)
    ...
    # Il funzionale è l'errore (differenza) tra l'immagine generata dalla popolazione e l'immagine originale.
    # Il minimo globale è costituito da una immagine nera, ovvero una immagine che contiene tutti zeri (in scala di grigi)  
    # Questo perché la differenza di due immagini uguali restituisce una immagine nera (tutti valori nulli della matrice)
    def __calcError(self, DNASeq, inImg):
        #draw the DNA
        myImg = self.drawAll(DNASeq)

        # E' possibile vedere le immagini con   cv2.imshow('image',diff1)
        # Effettua una doppia sottrazione, invertendo gli operandi, per enfatizzare i valori dell'immagine errore totalDiff.
        # NB: invertire gli operandi e sommare, in questo caso, non ritorna matematicamente valori nulli perchè l'operazione 
        # di sottrazione tra immagini non ritorna valori nulli. Ovvero, ad sempio, 56 - 100 darebbe zero, e non -44. Quindi
        # (56 - 100) + (100 - 56) = 0 + 44 = 44
        diff1 = cv2.subtract(inImg, myImg) 
        diff2 = cv2.subtract(myImg,inImg) 
        totalDiff = cv2.add(diff1, diff2)
        # Trasforma l'immagine errore in un numero che costituisce il risultato del funzionale da minimizzare
    ...
    def __evolveDNA(self, index, inImg, seed):
        ...
        changeIndices = []
        # Decide quanti geni modificare
        changeCount = random.randrange(1, len(indexOptions)+1)
        # Crea la lista degli indici dei geni da modificare
        ...
        # Inverte la lista degli indici dei geni da modificare
        ...
        # Se la mutazione generea un figlio migliore, allora viene sostituito al cromosoma di origine
...
# Pittura grossolana (esempio matlab con rand ed rgn('shuffle'))
gen = GeneticDrawing('example2.jpg', seed=time.time())
out = gen.generate(20, 20)

# Pittura fine rimpiccolendo il pennello e caricando una maschera per concentrarsi sui dettagli dell'immagine
...
imageio.mimsave('./movie.gif', gen.imgBuffer,duration = 0.3)